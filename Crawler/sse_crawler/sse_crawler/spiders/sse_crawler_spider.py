...
from sse_crawler.items import SseCrawlerItem
from scrapy.selector import Selector

class SseCrawlerSpider(scrapy.Spider):
    name = "sse_crawler"
    ...
    def __init__(self):
        f = open("/root/originaltech/crawler/sse_crawler/sse_crawler/spiders/crawled_urls", "r")
        line = f.readline()
        while line != "":
            line = line.strip("\n")
    ...
    def parse(self, response):
        ...
        if lower_url.endswith("htm") or lower_url.endswith("html"):
            item = SseCrawlerItem()
            item['title'] = selector.xpath('//div[@class="article-infor"]/h2/text()').extract_first()
            ...
            content_list = selector.xpath('//div[@class="allZoom"]/p/text()').extract()
            item['content'] = " ".join(content_list).replace('\u3000', '')
            if item['title'] != None:
                item['time'] = selector.xpath('//div[@class="article_opt"]/i/text()').extract_first()
        ...
        for sel in selector.xpath('//a'):
            ...
            if link != None and not link.endswith("pdf"):
                ...
                if url != None and  url.lower().find("sse.com.cn") != -1:
